 ...
 *
 */
public class TelestaxProxyAlgorithmMock extends CallIDAffinityBalancerAlgorithm {

    private static Logger logger = Logger.getLogger(TelestaxProxyAlgorithmMock.class.getCanonicalName());
...
//            return super.processExternalRequest(request);
    ...
    public void proxyMessage(ChannelHandlerContext ctx, MessageEvent e){
        ...
        if (uri.startsWith("https")) {
            f = cb.connect(new InetSocketAddress("127.0.0.1", 8443));
        } else {
            f = cb.connect(new InetSocketAddress("127.0.0.1", 8090));
        }
