...
checks whether http request contains 'CallSID' parameter. If it does, the Load balancer checks whether 'instanceId' from http request corresponds to 'instanceId'
of each node connected to the Load balancer. If the Load balancer finds the correspondence, it sends http request to this node.
