...
+
Configure the {this-platform} Load Balancer's Configuration Properties file by substituting valid values for your personal setup. <<sample_lb.properties_file>>          shows a sample [path]_lb-configuration.xml_ file, with key element descriptions provided after the example.
The lines beginning with the pound sign are comments.
+
[[sample_lb.properties_file]]
.Sample of lb-configuration.xml file
====
[source]
----
...
<mgcp>
  <mgcpExternalHost>50.17.127.170</mgcpExternalHost>
  <mgcpInternalHost>172.21.0.105</mgcpInternalHost>
  <mgcpExternalPort>2727</mgcpExternalPort>
  <mgcpInternalPort>2727</mgcpInternalPort>
</mgcp>
<ssl>
