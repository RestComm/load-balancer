...
The Load balancer has SS7 load balancing module. SS7 load balancer allows
balance SS7 traffic on level the level of SCCP protocol. For balancing it uses
round robin algorithm. For routing on internal side the Load balancer use routing
based on the point code, for routing on external side global title. After getting
very first SCCP request (BEGIN) LB gets dialogId and global title from request, choose point code
of available internal nodes via algorithm and stores it in map (or infinispan cache)
< dialogId,GT -> PointCode >. Each entity of map has time stamp, this stamp updates
after gating subsequence request. So the next sub requests which will be belong to
same dialog will be send to same node. All requests from internal side will be send
accordingly to the global title. All entity of the map will be removed after getting
ABORT/END request or after expiration time.

.SS7 load balancer diagram
image::images/SS7-lb-diagram.png[]

Configuration of the SS7 Balancer can be split on three main parts:

 * common part
  - nodeExpirationTaskInterval - period in msec in which LB checks is node alive.
  - nodeExpiration - max period in msec in which LB may not get ping from the Node
  and not remove it from available
  - periodCheckAlgoMap - period in msec in which LB checks old entity in map
  < dialogId,GT -> PointCode >.
  - maxIdleTime - max expiration time in sec in which entity can be stored in map without update
  of time stamp.
  - ss7AlgorithmClass - by default it is round robin.
 * external configuration - it is static configuration of SS7 external stack
 and will not change during all work of the Load balancer
 * internal cinfiguration - has some basic configuration for starting SS7 internal stack.
 After start and getting ping from internal SS7 Nodes LB modifies configuration of internal stack.

Here is example of configuration of the Load Balancer with SS7 module:
