...
The Load balancer will create array of nodes based on weight of this node. So if
node will have weight = 3, this node will present 3 times in this array. Max of
weight sets via 'maxWeightIndex' property. 'trafficRampupCyclePeriod' it's period in wich LB will
update array of nodes and its weight. This will be work in all algorithms.
...
maxWeightIndex::
Uses if rump-up needed. Max weight of node.
