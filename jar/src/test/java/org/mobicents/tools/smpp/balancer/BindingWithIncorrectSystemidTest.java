...
public class BindingWithIncorrectSystemidTest {

	private static final Logger logger = LoggerFactory.getLogger(BindingWithIncorrectSystemidTest.class);
	...
	private class Load extends Thread {
		...
		public void run() {
			...
			try 
			{
				...
				client.bind(sessionConfig, new DefaultSmppClientHandler());
			} catch (Exception e) {
				logger.error("Exception we will try to rebind");
				try {
					sleep(3000);
